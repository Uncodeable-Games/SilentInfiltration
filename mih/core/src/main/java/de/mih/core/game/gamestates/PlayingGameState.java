	...
	public void render()
	{
		...
		if (game.isEditMode())
		{
			font.draw(game.getRenderManager().spriteBatch, "EDIT MODE - (F10) to calculate Navigation (F11) to save (F12) to close", 10,
					464);
			font.draw(game.getRenderManager().spriteBatch, "(w) place/remove wall", 10, 448);
			font.draw(game.getRenderManager().spriteBatch, "(s) place/remove halfwall", 10, 432);
			font.draw(game.getRenderManager().spriteBatch, "(d) place/remove door", 10, 416);
			font.draw(game.getRenderManager().spriteBatch, "(a) place/remove halfdoor", 10, 400);
		}
