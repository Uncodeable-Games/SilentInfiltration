...
public class CircularContextMenuRenderer extends BaseRenderer {
	...
	public void render() {
		for (CircularContextMenuButton button: CircularContextMenu.getInstance().buttons){
			RenderManager.getInstance().spriteBatch.draw(button.icon, button.pos.x-button.iconsize, Gdx.graphics.getHeight()-button.pos.y-button.iconsize, 2*button.iconsize,2*button.iconsize);
		}
	}
}
