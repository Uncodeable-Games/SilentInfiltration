...
public class Room
{
	...
	public void calculateVisibility()
	{
		...
		{
			if (Game.getCurrentGame().getEntityManager().hasComponent(i, ColliderC.class)
					&& !Game.getCurrentGame().getEntityManager().hasComponent(i, VelocityC.class))
			{
	...
	public void calculateCenter()
	{
		for (Tile tile : tiles)
		{
			...
			if (leftTop == null
					|| (isLeft(tile.getCenter(), leftTop.getCenter()) && isOver(tile.getCenter(), leftTop.getCenter())))
				leftTop = tile;
			if (rightTop == null || (isRight(tile.getCenter(), rightTop.getCenter())
					&& isOver(tile.getCenter(), rightTop.getCenter())))
				rightTop = tile;
			if (leftBottom == null || (isLeft(tile.getCenter(), leftBottom.getCenter())
					&& isUnder(tile.getCenter(), leftBottom.getCenter())))
				leftBottom = tile;
			if (rightBottom == null || (isRight(tile.getCenter(), rightBottom.getCenter())
					&& isUnder(tile.getCenter(), rightBottom.getCenter())))
				rightBottom = tile;
		}
