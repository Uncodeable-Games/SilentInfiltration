...
public class NavPoint {
	...
	public void calculateVisibility(Room r) {
		...
		System.out.println(this+"("+pos.x+","+pos.y+")");
		for (ColliderC col1 : allcolliders) {
			for (NavPoint nav : col1.navpoints) {
				if (nav == this) {
					continue;
				}
				System.out.print("checking: "+nav+"("+nav.pos.x+","+nav.pos.y+")");
				intersects = false;
				...
				if (!intersects) {
					...
					visibleNavPoints.put(nav, (float) Math.sqrt(((pos.x - nav.pos.x) * (pos.x - nav.pos.x)
							+ (pos.y - nav.pos.y) * (pos.y - nav.pos.y))));
				}
			}
		}
	}
